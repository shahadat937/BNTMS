...
{
    public class GetInstructorBySchoolAndCourseRequestHandler : IRequestHandler<GetInstructorBySchoolAndCourseRequest, List<BnaExamInstructorAssignDto>>
    {
        private readonly ISchoolManagementRepository<BnaExamInstructorAssign> _BnaExamInstructorAssignRepository;

        private readonly IMapper _mapper;
        public GetInstructorBySchoolAndCourseRequestHandler(ISchoolManagementRepository<BnaExamInstructorAssign> BnaExamInstructorAssignRepository, IMapper mapper)
        {
            _BnaExamInstructorAssignRepository = BnaExamInstructorAssignRepository;
            _mapper = mapper;
        }

        public async Task<List<BnaExamInstructorAssignDto>> Handle(GetInstructorBySchoolAndCourseRequest request, CancellationToken cancellationToken)
        {
            IQueryable<BnaExamInstructorAssign> BnaExamInstructorAssigns = _BnaExamInstructorAssignRepository.FilterWithInclude(x => x.BaseSchoolNameId == request.BaseSchoolNameId && x.CourseNameId == request.CourseNameId && x.ClassRoutine.Date >= DateTime.Now, "BaseSchoolName", "BnaInstructorType", "BnaSubjectName", "ClassRoutine", "CourseDuration", "CourseModule", "CourseName", "Trainee.Rank").OrderByDescending(x => x.BnaExamInstructorAssignId);
