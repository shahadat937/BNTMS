...
{
    public class UpdateTraineeNominationListCommandHandler : IRequestHandler<UpdateTraineeNominationListCommand, BaseCommandResponse>
    {
        private readonly IUnitOfWork _unitOfWork;
        private readonly IMapper _mapper;

        public UpdateTraineeNominationListCommandHandler(IUnitOfWork unitOfWork, IMapper mapper)
        {
            _unitOfWork = unitOfWork;
            _mapper = mapper;
        }

        public async Task<BaseCommandResponse> Handle(UpdateTraineeNominationListCommand request, CancellationToken cancellationToken)
        {
            var response = new BaseCommandResponse();

            var trainee = request.TraineeNominationListDto;

            var traineeList = trainee.TraineeListForm.Select(x => new TraineeNomination()
            {
                ...
                CertificateSerialNo = x.CertificateSerialNo,
