...
namespace SchoolManagement.Application.Features.RoleFeatures.Handler.Queries
{
    ...
    { 
        ...
        //    var roleIds = branches.Select(x => x.RoleId).Distinct().ToList();

        //    var rolesCollection = await _roleRepository.GetAll();
        //    var roles =  _roleRepository.Where(r => roleIds.Contains(r.Id)).ToDictionary(r => r.Id, r => r.Name);

        //    var featureKeys = branches.Select(x => x.FeatureKey).Distinct().ToList();
        //    var features = _featureRepository.Where(f => featureKeys.Contains(f.FeatureId)).ToDictionary(f=> f.FeatureId, f=> f.FeatureName);
