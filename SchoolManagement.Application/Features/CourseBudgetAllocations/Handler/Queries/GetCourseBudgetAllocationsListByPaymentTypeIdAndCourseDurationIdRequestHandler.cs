...
{
    public class GetCourseBudgetAllocationsListByPaymentTypeIdAndCourseDurationIdRequestHandler : IRequestHandler<GetCourseBudgetAllocationsListByPaymentTypeIdAndCourseDurationIdRequest, PagedResult<CourseBudgetAllocationDto>>
    { 
        ...
        public GetCourseBudgetAllocationsListByPaymentTypeIdAndCourseDurationIdRequestHandler(ISchoolManagementRepository<CourseBudgetAllocation> CourseBudgetAllocationRepository, IMapper mapper)
        {
            _CourseBudgetAllocationRepository = CourseBudgetAllocationRepository;
            _mapper = mapper;
        }

        public async Task<PagedResult<CourseBudgetAllocationDto>> Handle(GetCourseBudgetAllocationsListByPaymentTypeIdAndCourseDurationIdRequest request, CancellationToken cancellationToken)
        {
            ...
            IQueryable<CourseBudgetAllocation> CourseBudgetAllocationes = _CourseBudgetAllocationRepository.FilterWithInclude(x => String.IsNullOrEmpty(request.QueryParams.SearchText), "Trainee", "BudgetCode", "PaymentType").Where(x=>x.CourseDurationId ==request.CourseDurationId && x.PaymentTypeId == 2);
