...
{
    public class GetSelectedSubjectMarkByParametersRequestHandler : IRequestHandler<GetSelectedSubjectMarkByParametersRequest, List<SubjectMarkDto>>
    {
        ...
        public GetSelectedSubjectMarkByParametersRequestHandler(ISchoolManagementRepository<SubjectMark> SubjectMarkRepository, IMapper mapper)
        {
            _SubjectMarkRepository = SubjectMarkRepository;
            _mapper = mapper;
        }

        public async Task<List<SubjectMarkDto>> Handle(GetSelectedSubjectMarkByParametersRequest request, CancellationToken cancellationToken)
        {
            IQueryable<SubjectMark> SubjectMarks = _SubjectMarkRepository.FilterWithInclude(x => x.BaseSchoolNameId == request.BaseSchoolNameId && x.CourseNameId == request.CourseNameId && x.CourseModuleId == request.CourseModuleId && x.BnaSubjectNameId == request.BnaSubjectNameId, "BaseSchoolName", "BnaSubjectName", "CourseModule", "CourseName", "MarkType").OrderByDescending(x => x.SubjectMarkId);
