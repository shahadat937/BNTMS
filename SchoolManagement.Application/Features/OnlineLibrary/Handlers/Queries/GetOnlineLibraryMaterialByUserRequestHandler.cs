...
{
    public class GetOnlineLibraryMaterialByUserRequestHandler : IRequestHandler<GetOnlineLibraryMaterialByUserRequest, PagedResult<OnlineLibraryDto>>
    {
        private readonly ISchoolManagementRepository<SchoolManagement.Domain.OnlineLibrary> _onlineLibrary;

        private readonly IMapper _mapper;

        public GetOnlineLibraryMaterialByUserRequestHandler(ISchoolManagementRepository<SchoolManagement.Domain.OnlineLibrary> onlineLibrary, IMapper mapper)
        {
            _onlineLibrary = onlineLibrary;
            _mapper = mapper;
        }
        public async Task<PagedResult<OnlineLibraryDto>> Handle(GetOnlineLibraryMaterialByUserRequest request, CancellationToken cancellationToken)
        {
            ...
            IQueryable<SchoolManagement.Domain.OnlineLibrary> OnlineLibrarys = _onlineLibrary.FilterWithInclude(x => x.CreatedBy == request.UserId && (String.IsNullOrEmpty(request.QueryParams.SearchText) || x.DocumentName.Contains(request.QueryParams.SearchText)), "BaseSchoolName", "DocumentType", "DownloadRight", "ShowRight");
