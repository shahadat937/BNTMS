...
{
    public class GetTraineeBioDataGeneralInfoListCivilInstructorRequestHandler : IRequestHandler<GetTraineeBioDataGeneralInfoListCivilInstructorRequest, PagedResult<TraineeBioDataGeneralInfoDto>>
    {
        ...
        public GetTraineeBioDataGeneralInfoListCivilInstructorRequestHandler(ISchoolManagementRepository<SchoolManagement.Domain.TraineeBioDataGeneralInfo> TraineeBioDataGeneralInfoRepository, IMapper mapper)
        {
            _TraineeBioDataGeneralInfoRepository = TraineeBioDataGeneralInfoRepository;
            _mapper = mapper;
        }

        public async Task<PagedResult<TraineeBioDataGeneralInfoDto>> Handle(GetTraineeBioDataGeneralInfoListCivilInstructorRequest request, CancellationToken cancellationToken)
        {
            ...
            IQueryable<SchoolManagement.Domain.TraineeBioDataGeneralInfo> TraineeBioDataGeneralInfos = _TraineeBioDataGeneralInfoRepository.FilterWithInclude(x => (x.OfficerTypeId ==3) && ((x.Pno.Contains(request.QueryParams.SearchText) || String.IsNullOrEmpty(request.QueryParams.SearchText))), "BnaBatch", "TraineeStatus", "Rank", "Country");
