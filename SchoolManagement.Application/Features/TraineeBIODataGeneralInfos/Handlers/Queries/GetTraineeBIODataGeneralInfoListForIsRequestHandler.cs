...
{
    public class GetTraineeBioDataGeneralInfoListForIsRequestHandler : IRequestHandler<GetTraineeBioDataGeneralInfoListForIsRequest, PagedResult<TraineeBioDataGeneralInfoDto>>
    {
        ...
        public GetTraineeBioDataGeneralInfoListForIsRequestHandler(ISchoolManagementRepository<SchoolManagement.Domain.TraineeBioDataGeneralInfo> TraineeBioDataGeneralInfoRepository, IMapper mapper)
        {
            _TraineeBioDataGeneralInfoRepository = TraineeBioDataGeneralInfoRepository;
            _mapper = mapper;
        }

        public async Task<PagedResult<TraineeBioDataGeneralInfoDto>> Handle(GetTraineeBioDataGeneralInfoListForIsRequest request, CancellationToken cancellationToken)
        {
            ...
            IQueryable<SchoolManagement.Domain.TraineeBioDataGeneralInfo> TraineeBioDataGeneralInfos = _TraineeBioDataGeneralInfoRepository.FilterWithInclude(x => x.TraineeStatusId ==8 && ((x.Pno.Contains(request.QueryParams.SearchText) || String.IsNullOrEmpty(request.QueryParams.SearchText))), "BnaBatch", "TraineeStatus", "Rank", "Country");
