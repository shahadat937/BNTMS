...
{
    public class GetAutoCompleteTraineeBioDataGeneralInfoRequestHandler : IRequestHandler<GetAutoCompleteTraineeBioDataGeneralInfoRequest, List<SelectedModel>>
    {
        ...
        public GetAutoCompleteTraineeBioDataGeneralInfoRequestHandler(
        ...
        public async Task<List<SelectedModel>> Handle(GetAutoCompleteTraineeBioDataGeneralInfoRequest request, CancellationToken cancellationToken)
        {
            var selectModels = new List<SelectedModel>();
            var traineeNominations = _traineeNominationRepository.FilterWithInclude(x => x.CourseDurationId == request.CourseDurationId && x.CourseNameId == request.CourseNameId && x.Trainee.Pno == request.PNo, "Trainee").ToList();
