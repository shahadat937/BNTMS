...
{
    public class GetTraineeBioDataGeneralInfoListForMidRequestHandler : IRequestHandler<GetTraineeBioDataGeneralInfoListForMidRequest, PagedResult<TraineeBioDataGeneralInfoDto>>
    {
        ...
        public GetTraineeBioDataGeneralInfoListForMidRequestHandler(ISchoolManagementRepository<SchoolManagement.Domain.TraineeBioDataGeneralInfo> TraineeBioDataGeneralInfoRepository, IMapper mapper)
        {
            _TraineeBioDataGeneralInfoRepository = TraineeBioDataGeneralInfoRepository;
            _mapper = mapper;
        }

        public async Task<PagedResult<TraineeBioDataGeneralInfoDto>> Handle(GetTraineeBioDataGeneralInfoListForMidRequest request, CancellationToken cancellationToken)
        {
            ...
            IQueryable<SchoolManagement.Domain.TraineeBioDataGeneralInfo> TraineeBioDataGeneralInfos = _TraineeBioDataGeneralInfoRepository.FilterWithInclude(x => x.TraineeStatusId ==7 && ((x.Pno.Contains(request.QueryParams.SearchText) || String.IsNullOrEmpty(request.QueryParams.SearchText))), "BnaBatch", "TraineeStatus", "Rank", "Country");
