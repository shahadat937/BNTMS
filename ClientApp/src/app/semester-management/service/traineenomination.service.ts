...
export class TraineeNominationService {
  ...
  //autocomplete by courseDuration & CourseName
  ...
  //autocomplete by BnasemesterDuration, courseDuration & CourseName
    //trainee-bio-data-general-info/get-autocompletePnoForNbcdNomination?pno=3208
    // getSelectedTraineeByparameterForBnaRequest(pno,bnaSemesterDurationId,courseDurationId,courseNameId){
    //   return this.http.get<SelectedModel[]>(this.baseUrl + '/trainee-bio-data-general-info/get-autocompleteForBnaTraineeByPno?pNo='+pno+'&bnaSemesterDurationId='+bnaSemesterDurationId+'&courseDurationId='+courseDurationId+'&courseNameId='+courseNameId)
    //     .pipe(
    //       map((response:[]) => response.map(item => item))
    //     )
    // }
    //autocomplete by BnasemesterDuration, courseDuration & CourseName
    //trainee-bio-data-general-info/get-autocompletePnoForNbcdNomination?pno=3208
    getSelectedTraineeByparameterForBnaRequest(pno){
  ...
  gettraineeNominationListByBnaSemesterDurationId(bnaSemesterDurationId){
    return this.http.get<any[]>(this.baseUrl + '/trainee-nomination/get-traineeNominationListForBnaByBnaSemesterDurationIdspRequest?bnaSemesterDurationId='+bnaSemesterDurationId)
  }
  ...
  getTraineeNominationsByBnaSemesterDurationId(pageNumber, pageSize,searchText,bnaSemesterDurationId) {
    ...
    params = params.append('pageNumber', pageNumber.toString());
    params = params.append('pageSize', pageSize.toString());
    params = params.append('bnaSemesterDurationId', bnaSemesterDurationId.toString());
    
    return this.http.get<ITraineeNominationPagination>(this.baseUrl + '/trainee-nomination/get-traineeNominationsByBnaSemesterDurationId', { observe: 'response', params })
    .pipe(
      map(response => {
        this.TraineeNominations = [...this.TraineeNominations, ...response.body.items];
        this.TraineeNominationPagination = response.body;
