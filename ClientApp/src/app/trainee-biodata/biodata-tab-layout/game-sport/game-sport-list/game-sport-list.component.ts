...
export class GameSportListComponent implements OnInit, OnDestroy {
  ...
  getGameSports() {
    ...
    this.subscription = this.GameSportService.getdatabytraineeid(+this.traineeId).subscribe(response => {     
  ...
  deleteItem(row) {
    ...
    this.subscription = this.confirmService.confirm('Confirm delete message', 'Are You Sure Delete This Item').subscribe(result => {
      if (result) {
        this.subscription = this.GameSportService.delete(id).subscribe(() => {
          this.getGameSports();
