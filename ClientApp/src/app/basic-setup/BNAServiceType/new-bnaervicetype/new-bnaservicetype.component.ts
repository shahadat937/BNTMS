...
export class NewBNAServiceTypeComponent
  ...
  implements OnInit
{
  ...
  ngOnInit(): void {
    ...
    if (id) {
      ...
      this.btnText = "Update";
      this.BNAServiceTypeService.find(+id).subscribe((res) => {
    ...
    if (id) {
      this.confirmService
        ...
        .subscribe((result) => {
          if (result) {
            this.loading = true;
            this.BNAServiceTypeService.update(
              +id,
              this.BNAServiceTypeForm.value
            ).subscribe(
              (response) => {
                this.router.navigateByUrl("/basic-setup/bnaservicetype-list");
                this.snackBar.open(
                  "BNA ServiceType Information Updated Successfully ",
                  "",
                  {
                    duration: 2000,
                    verticalPosition: "bottom",
      ...
      this.loading = true;
      this.BNAServiceTypeService.submit(
        this.BNAServiceTypeForm.value
      ).subscribe(
        (response) => {
          this.router.navigateByUrl("/basic-setup/bnaservicetype-list");
