...
export class NewCourseNameComponent
  ...
  implements OnInit
{
  ...
  ngOnInit(): void {
    ...
    if (id) {
      ...
      this.btnText = "Update";
      this.CourseNameService.find(+id).subscribe((res) => {
  ...
  filterByType(value: any) {
    this.selectedCourseType = this.selectCourse.filter((x) =>
    ...
    if (id) {
      this.confirmService
        ...
        .subscribe((result) => {
          if (result) {
            this.loading = true;
            this.CourseNameService.update(+id, formData).subscribe(
              (response) => {
      ...
      this.loading = true;
      this.CourseNameService.submit(formData).subscribe(
        (response) => {
          this.router.navigateByUrl("/basic-setup/coursename-list");
