...
export class BNASubjectCurriculamListComponent extends UnsubscribeOnDestroyAdapter implements OnInit {
  ...
  constructor(private snackBar: MatSnackBar,private BNASubjectCurriculamService: BNASubjectCurriculamService,private router: Router,private confirmService: ConfirmService) {
    super();
  }
  ...
  getBNASubjectCurriculams() {
    this.isLoading = true;
    this.BNASubjectCurriculamService.getBNASubjectCurriculams(this.paging.pageIndex, this.paging.pageSize,this.searchText).subscribe(response => {
    ...
    this.confirmService.confirm('Confirm delete message', 'Are You Sure Delete This  Item').subscribe(result => {
      if (result) {
        this.BNASubjectCurriculamService.delete(id).subscribe(() => {
          ...
          this.snackBar.open('BNA Subject Curriculum Deleted Successfully ', '', {
            duration: 2000,
