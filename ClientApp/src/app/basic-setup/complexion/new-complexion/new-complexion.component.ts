...
export class NewComplexionComponent
  ...
  implements OnInit
{
  ...
  ngOnInit(): void {
    ...
    if (id) {
      ...
      this.complexionService.find(+id).subscribe((res) => {
  ...
  onSubmit() {
    ...
    if (id) {
        ...
        .subscribe((result) => {
          if (result) {
            ...
            this.complexionService
              .update(+id, this.complexionForm.value)
              .subscribe(
                (response) => {
                  this.router.navigateByUrl("/basic-setup/complexion-list");
                  this.snackBar.open(
                    "Complexion Information Updated Successfully ",
                    "",
                    {
                      duration: 2000,
      ...
      this.loading = true;
      this.complexionService.submit(this.complexionForm.value).subscribe(
        (response) => {
          this.router.navigateByUrl("/basic-setup/complexion-list");
