...
export class NewBnaAttendancePeriodComponent
  ...
  implements OnInit
{
  ...
  ngOnInit(): void {
    ...
    if (id) {
      ...
      this.btnText = "Update";
      this.BnaAttendancePeriodService.find(+id).subscribe((res) => {
    ...
    if (id) {
      this.confirmService
        ...
        .subscribe((result) => {
          if (result) {
            this.loading = true;
            this.BnaAttendancePeriodService.update(
              +id,
              this.BnaAttendancePeriodForm.value
            ).subscribe(
              (response) => {
                this.router.navigateByUrl(
                  "/basic-setup/bnaattendanceperiod-list"
                );
      ...
      this.loading = true;
      this.BnaAttendancePeriodService.submit(
        this.BnaAttendancePeriodForm.value
      ).subscribe(
        (response) => {
