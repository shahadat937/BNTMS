...
export class NewAccountTypeComponent
  ...
  implements OnInit
{
  ...
  ngOnInit(): void {
    ...
    if (id) {
      ...
      this.btnText = "Update";
      this.AccountTypeService.find(+id).subscribe((res) => {
    ...
    if (id) {
      this.confirmService
        ...
        .subscribe((result) => {
          if (result) {
            this.loading = true;
            this.AccountTypeService.update(
              +id,
      ...
      this.loading = true;
      this.AccountTypeService.submit(this.AccountTypeForm.value).subscribe(
        (response) => {
          this.router.navigateByUrl("/basic-setup/accounttype-list");
