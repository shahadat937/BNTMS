...
export class NewBNABatchComponent
  ...
  ngOnInit(): void {
    ...
    if (id) {
      ...
      this.bNABatchService.find(+id).subscribe((res) => {
    ...
    if (id) {
      this.confirmService
        ...
        .subscribe((result) => {
          if (result) {
            this.loading = true;
            this.bNABatchService.update(+id, this.bNABatchForm.value).subscribe(
              (response) => {
      ...
      this.loading = true;
      this.bNABatchService.submit(this.bNABatchForm.value).subscribe(
        (response) => {
          this.router.navigateByUrl("/basic-setup/bnabatch-list");
