...
export class NewBNASemesterComponent
  ...
  implements OnInit
{
  ...
  ngOnInit(): void {
    ...
    if (id) {
      ...
      this.btnText = "Update";
      this.BNASemesterService.find(+id).subscribe((res) => {
    ...
    if (id) {
      this.confirmService
        ...
        .subscribe((result) => {
          if (result) {
            this.loading = true;
            this.BNASemesterService.update(
              +id,
              this.BNASemesterForm.value
            ).subscribe(
              (response) => {
      ...
      this.loading = true;
      this.BNASemesterService.submit(this.BNASemesterForm.value).subscribe(
        (response) => {
          this.router.navigateByUrl("/basic-setup/bnasemester-list");
