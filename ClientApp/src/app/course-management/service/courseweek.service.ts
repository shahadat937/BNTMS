...
export class CourseWeekService {
  ...
  getrunningCourseListBySchool(current, courseTypeId, schoolId) {
    return this.http.get<any[]>(this.baseUrl + '/dashboard/get-runningCourseDurationBySchool?courseTypeId='+courseTypeId+'&CurrentDate='+current+'&baseSchoolNameId='+schoolId).pipe(
      map(response => {
  ...
  getCourseWeeks(pageNumber, pageSize,searchText,baseSchoolNameId,courseDurationId) {
  ...
  ChangeCourseWeekStatus(courseWeekId, status) {
    return this.http.get<any>(this.baseUrl + '/course-week/change-courseWeekStatus?courseWeekId='+courseWeekId+'&Status='+status);
  }
}
