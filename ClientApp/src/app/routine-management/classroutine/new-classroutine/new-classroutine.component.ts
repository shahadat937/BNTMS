...
export class NewClassRoutineComponent implements OnInit, OnDestroy {
  ...
  addSinglePeriod(subjectId: string) {

    this.selectedsubjectname = this.selectSubjectName
    this.selectedclasstype = this.selectClassType
    ...
    // Find the max classCountPeriod for the same subject
    control.controls.forEach(group => {
      const existingSubjectId = group.get('subjectId')?.value;
      const classCount = group.get('classCountPeriod')?.value;

      if (existingSubjectId === subjectId) {
        const match = String(classCount).match(/^(\d+)/);
        if (match) {
    ...
    newPeriod.get('subjectId')?.setValue(subjectId);
    newPeriod.get('classCountPeriod')?.setValue(String(maxPeriod + 1));
  ...
  deletePeriod() {
    ...
    if (control?.length > 1) {
      ...
      this.disableDeleteBtn = control.length === 1;
    }
  ...
  onSubjectNameSelectionChangeGet(dropdown, index: number) {
    this.currentSelectedSubjectId = dropdown.value;
    ...
    this.updateClassCountPeriodOnSubjectChange(subjectId, index);
  ...
  updateClassCountPeriodOnSubjectChange(subjectId: string, currentIndex: number) {
    const control = this.ClassRoutineForm.get("perodListForm") as FormArray;

    let lastMatchingCount = 0;

    // Loop over previous rows and find max classCountPeriod
    for (let i = 0; i < currentIndex; i++) {
      const group = control.at(i);
      const classCountPeriod = group.get('classCountPeriod')?.value;
      if (group.get('subjectId')?.value === subjectId && classCountPeriod !== null) {
        const value = parseInt(classCountPeriod, 10);
        if (!isNaN(value) && value > lastMatchingCount) {
          lastMatchingCount = value;
        }
      }
    }

    const newCount = lastMatchingCount + 1 || 1;
    const currentGroup = control.at(currentIndex);
    const currentClassCount = currentGroup.get('classCountPeriod')?.value;


    // Only update if the value is different from the existing value
    if (currentClassCount !== newCount) {
      currentGroup.get('classCountPeriod')?.setValue(newCount);
    }
  ...
  onSubjectNameSelectionChange(baseSchoolNameId, courseNameId, courseSectionId, bnaSubjectNameId, courseDurationId, index) {
    ...
    const perodListForm = this.ClassRoutineForm.get('perodListForm') as FormArray;
    perodListForm.at(index).get('bnaSubjectNameId')?.setValue(bnaSubjectNameId);

    const subjectUsageCount = perodListForm.controls.filter(ctrl =>
      ctrl.get('bnaSubjectNameId')?.value == bnaSubjectNameId
    ).length;



    if (subjectUsageCount === 1) {
      this.subscription = this.ClassRoutineService
        ...
        .subscribe(res => {
          ...
          perodListForm.at(index).get('classCountPeriod')?.setValue(this.routineCount);
        });
    } else {
      this.updateClassCountPeriodOnSubjectChange(bnaSubjectNameId, index);
    }
  ...
  onSectionSelectionGet() {
    ...
    this.subscription = this.ClassRoutineService.getClassRoutineByCourseNameBaseSchoolNameSpRequest(baseSchoolNameId, courseNameId, courseWeekId, this.sectionId).subscribe(res => {
      ...
      if (this.selectedRoutineByParametersAndDate[0]) {
