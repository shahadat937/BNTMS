...
export class AddBnaClassattendanceComponent
  ...
  implements OnInit
{
  ...
  getControlLabel(index: number, type: string) {
    return (this.BnaAttendanceForm.get("studentAttendanceForm") as FormArray)
      ?.at(index)
      ?.get(type)?.value;
  }
  getTraineeListonClick() {
    const control = <FormArray>(
      this.BnaAttendanceForm.controls["studentAttendanceForm"]
    );
    for (let i = 0; i < this.attendanceTraineeList.length; i++) {
      control.push(this.createTraineeListData());
    }
    this.BnaAttendanceForm.patchValue({
      studentAttendanceForm: this.attendanceTraineeList,
    });
  }
  clearList() {
    const control = <FormArray>(
  ...
  ViewTraineeListForAttendance() {
    if (
      ...
      this.AttendanceService.getAttendanceTraineeList(
        this.baseSchoolId,
        this.selectedbnaSubjectCurriculum,
        this.selectedcourseName,
        this.selectedcourseDuration,
        this.selectedbnaSemester,
  ...
  getselectedcourseTitle() {
    this.AttendanceService.getselectedcourseTitle(this.baseSchoolId).subscribe(
      (res) => {
  ...
  getDropdownCourseSection() {
    this.AttendanceService.getDropdownCourseSection(
      this.baseSchoolId
    ).subscribe((res) => {
  ...
  getSelectedClassPeriod() {
    this.AttendanceService.getDropdownClassPeriod(this.baseSchoolId).subscribe(
      (res) => {
  ...
  onDateSelectionChange(event) {
    this.selectedDate =
      this.datepipe.transform(event.value, "yyyy-MM-dd") + " 18:00:00.000";
    this.ViewTraineeListForAttendance();
  }
  ...
  onSubmit() {
    this.BnaAttendanceForm.value.bnaSubjectCurriculumId =
      this.selectedbnaSubjectCurriculum;
    ...
    if (this.actionStatus == "S") {
      this.confirmService
        .confirm("Confirm Save message", "Are You Sure Inserted This Records?")
        .subscribe((result) => {
          if (result) {
            this.loading = true;
            this.AttendanceService.bnaAttendanceSubmit(
              this.BnaAttendanceForm.value
            ).subscribe(
              (response) => {
      ...
      this.confirmService
        .confirm("Confirm Save message", "Are You Sure Update This Records?")
        .subscribe((result) => {
          if (result) {
            this.loading = true;
            this.AttendanceService.bnaAttendanceUpdate(
              this.BnaAttendanceForm.value
            ).subscribe(
              (response) => {
                this.reloadCurrentRoute();
