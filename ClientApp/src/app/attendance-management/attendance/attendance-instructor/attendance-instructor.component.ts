...
export class AttendanceInstructorComponent
  ...
  implements OnInit
{
  ...
  getselectedclassperiod(traineeId) {
    this.AttendanceService.getSelectedClassPeriodForAttendanceInstructorBySpRequest(
      traineeId
    ).subscribe((res) => {
  ...
  onClassPeriodSelectionChangeGetCourseDuration(dropdown) {
    if (dropdown.isUserInput) {
      ...
      this.traineeNominationService
        ...
        .subscribe((res) => {
          this.traineeNominationListForAttendance = res.filter(
            (x) => x.withdrawnTypeId === null
          );

          for (
            let i = 0;
            i < this.traineeNominationListForAttendance.length;
            i++
          ) {
  ...
  onSubmit() {
    ...
    var data = this.AttendanceForm.value.traineeListForm.filter(
      (x) => x.attendanceStatus === false && x.bnaAttendanceRemarksId === null
    );
    if (data.length > 0) {
      this.popup = true;
    } else {
